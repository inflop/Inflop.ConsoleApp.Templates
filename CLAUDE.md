...
- Template 4 (Enterprise) uses centralized configuration class (`Configuration/ServiceConfiguration.cs`)
...
```bash
# Pack the templates (IncludeBuildOutput=false prevents compilation)
dotnet pack src/Inflop.ConsoleApp.Templates.csproj -c Release
```
The output .nupkg file will be in `src/bin/Release/`.

**Why templates aren't compiled:** The template project has `IncludeBuildOutput=false` in .csproj, which prevents MSBuild from compiling the template files. Template files contain conditional compilation directives (`//#if`, `//#endif`) that are processed by the template engine during `dotnet new`, not by the C# compiler.
...
- All target net8.0 (LTS) by default, with net9.0 available via template parameter
...
**Shared Files** (edit in `src/_shared/` → run `./scripts/sync-shared-files.sh`):
...
3. Run `./scripts/sync-shared-files.sh` (if shared file modified)
